  ...
  config =
    ...
    lib.mkIf cfg.enable {
      services.reaction = {
        ...
        runAsRoot = true;
        settingsFiles =
          lib.optional cfg.enableGoogleImpersonator (replaceActions {
            name = "googlebot.jsonnet";
            file = ./googlebot.jsonnet;
            actions = (var.banFor "30d");
          })
          ++ lib.optional cfg.enableGPTBot (replaceActions {
            name = "ai-robots.jsonnet";
            file = ./ai-robots.jsonnet;
            actions = (var.banFor "30d");
          });
        settings = {
          patterns = {
            ...
            systemd = lib.mkIf cfg.enableSystemd {
              ...
              filters = {
                unit = {
                  ...
                  duplicate = "ignore";
                  actions = {
                    mail = var.mailMe "${config.networking.hostName}: unit <unit> failed" config.ppom.monit;
                    dummy = {
                      cmd = [ "true" ];
                      after = "6h";
                    };
