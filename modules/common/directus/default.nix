...
let
  ...
  settingsJson =
    settings: json.generate "config.json" (lib.filterAttrs (key: value: value != null) settings);
in
{
                  ...
                  type = submodule {
                    ...
                    options = {
                      ...
                      PUBLIC_URL = mkOption {
                        type = str;
                        default =
                          if config.nginx.enable then "https://${config.nginx.serverName}${config.nginx.location}" else "/";
                      ...
                      DB_FILENAME = mkOption {
                        type = nullOr str;
                        default =
                          if config.settings.DB_CLIENT == "sqlite3" then "/var/lib/directus-${name}/data.db" else null;
                      ...
                      nginx = mkOption {
                        ...
                        type = submodule (
                          recursiveUpdate (import
                            /nix/var/nix/profiles/per-user/root/channels/nixos/nixos/modules/services/web-servers/nginx/vhost-options.nix
                            { inherit config lib; }
                          ) { }
                        );
                        default = { };
        ...
        })
        {
          ...
          assertions =
            ...
            ++ lib.mapAttrsToList (name: conf: {
              assertion =
                conf.nginx.location != ""
                -> "/" != builtins.substring (-1 + builtins.stringLength conf.nginx.location) 1 conf.nginx.location;
              message = "services.directus.servers.<name>.nginx.location musn't end with a '/'";
            }) enabledServers
            ++ [
              {
                assertion =
                  cfg.allowDirectusLicense
                  || (builtins.hasAttr "allowUnfree" config.nixpkgs.config && config.nixpkgs.config.allowUnfree);
          ...
          users.users = lib.mapAttrs' (
            name: conf:
            lib.nameValuePair "directus-${name}" {
          ...
          systemd.tmpfiles.rules =
            ...
            in
            lib.mapAttrsToList (
              name: conf: "d /var/lib/directus-${name}/secrets   0750 directus-${name} directus-${name} - -"
            ) enabledServers
            ++ lib.mapAttrsToList (
              name: conf: "d ${conf.settings.STORAGE_LOCAL_ROOT} 0750 directus-${name} directus-${name} - -"
            ) localStorageServers
          ...
          services.nginx.virtualHosts =
            let
              nginxEnabledServers = builtins.attrValues (
                lib.filterAttrs (name: conf: conf.nginx.enable) enabledServers
              );
              groupedByServerNameConfs = builtins.groupBy (conf: conf.nginx.serverName) nginxEnabledServers;
            in
            builtins.mapAttrs (name: confs: {
              enableACME = true;
              forceSSL = true;
              locations = lib.mkMerge (
                builtins.map (conf: {
                  "${conf.nginx.location}/".proxyPass = "http://localhost:${builtins.toString conf.settings.PORT}/";
                  "${conf.nginx.location}" = lib.mkIf (conf.nginx.location != "") {
                    return = "302 ${conf.nginx.location}/";
                  };
                }) confs
              );
