{ config, pkgs, ... }:
{
  ...
  # Configure low priority targets, which execute after the multi-user target.
  systemd.targets.multi-user2 = {
    enable = true;
    description = "Multi-User System, stage 2";
    wantedBy = [ "multi-user.target" ];
    after = [ "multi-user.target" ];
  };
  systemd.targets.multi-user3 = {
    enable = true;
    description = "Multi-User System, stage 3";
    wantedBy = [ "multi-user.target" ];
    after = [ "multi-user2.target" ];
  };
