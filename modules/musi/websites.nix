  ...
  services.nginx = {
    ...
    virtualHosts = {
      ...
      "blog.ppom.me" = {
        ...
        extraConfig = ''
          # add_header Strict-Transport-Security "max-age=31536000";
          add_header Content-Security-Policy "default-src 'self' u.ppom.me; img-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; frame-ancestors 'none'; base-uri 'none'; form-action 'none'";
          add_header X-Content-Type-Options "nosniff";
          add_header X-Frame-Options "DENY";
        '';
      };
