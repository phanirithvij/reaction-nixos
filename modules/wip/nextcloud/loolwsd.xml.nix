...
pkgs.writeText "loolwsd.xml" ''
<config>

    <!-- Note: 'default' attributes are used to document a setting's default value as well as to use as fallback. -->

    <allowed_languages desc="List of supported languages of Writing Aids (spell checker, grammar checker, thesaurus, hyphenation) on this instance. Allowing too many has negative effect on startup performance." default="de_DE en_GB en_US es_ES fr_FR it nl pt_BR pt_PT ru">${lib.concatStringsSep " " allowedLanguages}</allowed_languages>
    ...
    <server_name desc="External hostname:port of the server running loolwsd. If empty, it's derived from the request (please set it if this doesn't work). Must be specified when behind a reverse-proxy or when the hostname is not reachable directly." type="string" default="">${collaboraDomain}</server_name>
    ...
    <per_document desc="Document-specific settings, including LO Core settings.">
        ...
        <idlesave_duration_secs desc="The number of idle seconds after which document, if modified, should be saved. Defaults to 30 seconds." type="int" default="30">20</idlesave_duration_secs>
    ...
    <logging>
        ...
        <anonymize>
            ...
            <anonymization_salt type="uint" desc="The salt used to anonymize/obfuscate user-data in logs. Use a secret 64-bit random number." default="82589933">82684933</anonymization_salt>
        </anonymize>
    </logging>
    ...
    <storage desc="Backend storage">
        ...
        <wopi desc="Allow/deny wopi storage. Mutually exclusive with webdav." allow="true">
            <host desc="Regex pattern of hostname to allow or deny." allow="true">${nextcloudDomain}</host>
        ...
        <webdav desc="Allow/deny webdav storage. Mutually exclusive with wopi." allow="false">
            <host desc="Hostname to allow" allow="false">${nextcloudDomain}</host>
        </webdav>
    </storage>
    ...
    <admin_console desc="Web admin console settings.">
        <enable desc="Enable the admin console functionality" type="bool" default="true">${toString webadmin.enable}</enable>
